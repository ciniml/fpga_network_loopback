...
struct UDP_PseudoIPv4
{
	static constexpr const std::size_t SIZE = 12;
	std::array<std::uint8_t, SIZE> raw;
	
	UDP_PseudoIPv4(const IPv4 ip) {
		this->source(ip.source());
		this->destination(ip.destination());
		this->zero(0);
		this->protocol(ip.protocol());
	}

	IPAddress source() const                { return read_ipaddr(this->raw, 0); }
	IPAddress destination() const           { return read_ipaddr(this->raw, 4); }
	std::uint8_t zero() const         	    { return this->raw[8]; }
	std::uint8_t protocol() const           { return this->raw[9]; }
	std::uint16_t udp_length() const        { return read16be(this->raw, 10); }

	void source(const IPAddress& value)         { write_ipaddr(this->raw, 0, value); }
	void destination(const IPAddress& value)    { write_ipaddr(this->raw, 4, value); }
	void zero(std::uint8_t value)               { this->raw[8] = value; }
	void protocol(std::uint8_t value)           { this->raw[9] = value; }
	void udp_length(std::uint16_t value)        { write16be(this->raw, 10, value); }
};
...
struct UDP
{
	...
	std::array<std::uint8_t, SIZE> raw;
	std::uint16_t source_port() const { return read16be(this->raw, 0); }
	std::uint16_t destination_port() const { return read16be(this->raw, 2); }
	std::uint16_t length() const { return read16be(this->raw, 4); }
	std::uint16_t checksum() const { return read16be(this->raw, 6); }

	void source_port(std::uint16_t value) { write16be(this->raw, 0, value); }
	void destination_port(std::uint16_t value) { write16be(this->raw, 2, value); }
	void length(std::uint16_t value) { write16be(this->raw, 4, value); }
	void checksum(std::uint16_t value) { write16be(this->raw, 6, value); }

	void fill_checksum(const IPv4& ip, const std::uint16_t* payload, std::size_t payload_length)
	{
		UDP_PseudoIPv4 pseudo_ipv4(ip);
		pseudo_ipv4.udp_length(payload_length + SIZE);

		this->checksum(0);
		auto checksum = calculate_internet_checksum(pseudo_ipv4.raw);
		checksum = calculate_internet_checksum(this->raw, checksum);
		checksum = calculate_internet_checksum(payload, checksum, payload_length);
		this->checksum(~checksum);
	}
};
...
static void arp(const EthernetServiceConfig& config, const EthernetHeader& header, hls::stream<mac_data_axis>& in, hls::stream<mac_data_axis>& out)
{
	...
	if( arp.operation() != 0x0001 || compare_array(arp.tpa(), config.get_ip_address()) != 0 ) {
		return;
	}
...
static void icmp_reply(const EthernetServiceConfig& config, const EthernetHeader& header, const IPv4& src_ip, hls::stream<mac_data_axis>& in, hls::stream<mac_data_axis>& out)
{
	IPv4 ip = src_ip;
	ICMP icmp;
...
template<std::size_t MAX_PAYLOAD_LENGTH=1500>
static void udp(const EthernetServiceConfig& config, const EthernetHeader& header, const IPv4& src_ip, hls::stream<mac_data_axis>& in, hls::stream<mac_data_axis>& out)
{
	IPv4 ip = src_ip;
	UDP udp;
	std::array<std::uint16_t, MAX_PAYLOAD_LENGTH/2> payload;
//#pragma HLS BIND_STORAGE variable=payload type=ram_2p
	std::size_t payload_length = 0;
	{
		auto result =  read_exact(in, udp.raw);
		if( result == ReadExactResult::Remaining ) {
			payload_length = read_payload(in, reinterpret_cast<std::uint8_t*>(payload.data()), MAX_PAYLOAD_LENGTH);
		}
		if( result == ReadExactResult::NotEnough ) {
	...
	for( std::size_t i = 0; i < (payload_length+1)/2; i++ ) {
		payload[i] = payload[i] + 1;
	}

	// Construct UDP header
	auto original_destination_port = udp.destination_port();
	udp.destination_port(udp.source_port());
	udp.source_port(original_destination_port);
	udp.fill_checksum(ip, payload.data(), payload_length);
...
template<std::size_t MAX_PAYLOAD_LENGTH=1500>
static void ipv4(const EthernetServiceConfig& config, const EthernetHeader& header, hls::stream<mac_data_axis>& in, hls::stream<mac_data_axis>& out)
{
	IPv4 ip;
	{
	...
	if( compare_array(ip.destination(), config.get_ip_address()) != 0  ) {
		consume_remaining(in);
		return;
	}
	...
	{
		case 0x01: // ICMP
			icmp_reply(config, header, ip, in, out);
			break;
		case 0x11: // UDP
			udp(config, header, ip, in, out);
			break;
		default:
			consume_remaining(in);
			break;
